...
SRC_URI="
	...
	https://github.com/rust-lang/stacker/archive/refs/tags/stacker-${STACKER_PV}.tar.gz
...
src_unpack() {
	...
	cargo_src_unpack
	unpack stacker-${STACKER_PV}.tar.gz
}
...
src_prepare() {
	...
	echo "stacker = { path = '../stacker-stacker-${STACKER_PV}' }" >> Cargo.toml
	if use elibc_musl; then
		eapply "${FILESDIR}/fix-stackoverflow.patch"
		eapply "${FILESDIR}/glibc.patch"
	fi
	eapply "${FILESDIR}/ext-kv-fix-snapshot-failed.patch"
	popd > /dev/null || die
	pushd "${WORKDIR}/stacker-stacker-${STACKER_PV}" > /dev/null || die
