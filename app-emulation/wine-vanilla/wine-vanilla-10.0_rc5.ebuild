...
if [[ ${PV} == *9999 ]]; then
	...
	SRC_URI="https://dl.winehq.org/wine/source/${WINE_SDIR}/wine-${PV/_rc/-rc}.tar.xz"
	S="${WORKDIR}/wine-${PV/_rc/-rc}"
	KEYWORDS="-* ~amd64 ~x86"
fi
...
SLOT="${PV}"
IUSE="
	clang +X +abi_x86_32 +abi_x86_64 +alsa capi crossdev-mingw cups dos
...
pkg_pretend() {
	...
	if use crossdev-mingw && [[ ! -v MINGW_BYPASS ]]; then
		...
		for mingw in $(usev abi_x86_64 x86_64${mingw}) \
			...
			if ! type -P ${mingw}-${cc} >/dev/null; then
				...
				die "USE=crossdev-mingw is enabled, but ${mingw}-${cc} was not found"
			fi
		done
	fi
}
...
src_configure() {
	...
	if use mingw; then
		...
		local mingwcc_amd64=${CROSSCC:-${CROSSCC_amd64:-x86_64-w64-mingw32-${cc}}}
		local mingwcc_x86=${CROSSCC:-${CROSSCC_x86:-i686-w64-mingw32-${cc}}}
